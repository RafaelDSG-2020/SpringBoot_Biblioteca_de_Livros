...
public class AuthorController {
    ...
    @GetMapping("/author")
    public ResponseEntity<Object> findAuthorByIDExist(){

        log.info("Method: findAuthorByIDExist searches for just one author by their ID. HTTP Method: GET");
        long start = System.currentTimeMillis();
        AuthorDTO personSave = authorService.isAuthorAvailable();
        log.info("HTTP Method: GET Endpoint: api/v1/authors/author  elapsedTime = {} ms", (System.currentTimeMillis() - start));
        return  ResponseEntity.status(HttpStatus.OK).body(personSave);
