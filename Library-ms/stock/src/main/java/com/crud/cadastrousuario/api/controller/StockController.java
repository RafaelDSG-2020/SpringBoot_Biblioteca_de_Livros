...
public class StockController {
    ...
    @GetMapping("/{columnBook}")
    public ResponseEntity<Object> findBookID(@PathVariable Long columnBook){
        log.info("Method: findBookID find the book in stock by id. HTTP Method: GET");
        long start = System.currentTimeMillis();
        StockDTO bookStock = stockService.findBookID(columnBook);
        log.info("HTTP Method: GET Endpoint: api/v1/stock/{columnBook}  elapsedTime = {} ms" , (System.currentTimeMillis() - start));
        return ResponseEntity.status(HttpStatus.OK).body(bookStock);
